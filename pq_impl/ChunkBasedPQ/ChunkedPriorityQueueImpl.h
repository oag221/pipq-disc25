...
void ChunkedPriorityQueue::print()
{
	Chunk 	*curr 	= head;
	int 	counter	= 0;
...
/******************************************************************************************************/
// ChunkedPriorityQueue::debugKeySum() 	- prints each chunk of the priority queue
long ChunkedPriorityQueue::debugKeySum()
{
	Chunk *curr = head;
	long sum = 0;

	cout	<<	"head = "	<<	head	<<	endl;
	cout	<< 	"buffer = " << curr->meta.buffer << ".\nCalculating keysum..." << endl;
	if (curr->clearDelBuffer()) sum += curr->meta.buffer->keysum(true, false);

	bool first = true;
	while (curr != NULL) {
		if (first) {
			sum += curr->keysum(false, true);
			first = false;
		} else {
			sum += curr->keysum();
		}
...
// end of ChunkedPriorityQueue::debugKeySum()
/******************************************************************************************************/


/******************************************************************************************************/
// ChunkedPriorityQueue::deInit() 	- prints each chunk of the priority queue
long ChunkedPriorityQueue::deInit()
{
	return keysum;
	// Chunk 	*curr 	= head;
	// int 	counter	= 0;
...
// end of ChunkedPriorityQueue::deInit()
/******************************************************************************************************/
...
bool ChunkedPriorityQueue::insert(int key, ThrInf* t) {
	...
	while(1) {
		...
		if ( head == curr ){
			// PRINT("\nhead == curr\n");	
			// print();					// if this is the 1st chunk (being first is irreversible)
			_assert(curr->meta.status.isDelete() || curr->meta.status.isInFreeze());
			if(insert_to_first_chunk(t, key, curr)) {
				//std::cout << "breaking after insert to first chunk\n";
...
bool ChunkedPriorityQueue::insert_to_first_chunk(ThrInf* t, int key, Chunk *currhead){
	...
	if(currhead->meta.status.isInFreeze()){
		//PRINT("in freeze, returning true\n");
		...
		// PRINT("\nPrinting before freeze calls:\n");
		// print();
		// PRINT("\nCalling freeze chunk\n");
		freezeChunk(currhead, svals, &len, INSERT_FIRST_HEAD, t);
		//print();
		//PRINT("\nCalling freeze recovery\n");
...
int ChunkedPriorityQueue::delmin(ThrInf* t){
	...
	while(1){
		...
		if ( (idx<MAX_IDX) && (!Atomicable::isInFreeze(status)) ) {
			...
			//PRINT2("The minimum is %d from idx %d (Thread %d)\n", curr->vals[idx],idx, t->id);
...
void ChunkedPriorityQueue::readFrozenChunk(Chunk *curr, int sortedval[], int *len){
	...
	// printf("\nfrozen idx: %d\n", frozenIdx);
	// PRINT("\n\nbefore loop in readFrozenChunk():\n");
	// print();
	...
	// PRINT("\n\nafter loop in readFrozenChunk(): (j = %d)\n", j);
	// print();
