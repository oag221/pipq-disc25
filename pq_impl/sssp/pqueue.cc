...
// SCANINC is the amount to increase scan length at each step; can be any
// integer
#define SCANINC 0
...
static int _MarsagliaXOR(int seed) {
  // int const a =      16807;
  int const a = 123456789;
  int const m = 2147483647;
  int const q = 521288629; /* m div a */
  int const r = 88675123;  /* m mod a */
  int hi = seed / q;
  int lo = seed % q;
  int test = a * lo - r * hi;
  if (test > 0)
...
int spray_delete_min_key(sl_intset_t *set, slkey_t *key, val_t *val,
  ...
  *seed = _MarsagliaXOR(*seed);
  if (n == 1 || *seed % n /*/floor_log_2(n)*/ ==
                    0) { // n == 1 is equivalent to Lotan-Shavit delete_min
    d->nb_clean++;
    return lotan_shavit_delete_min_key(set, key, val, d);
  }
  ...
  while (1) {
    *seed = _MarsagliaXOR(*seed);
    ...
    while (scanlen > 0 &&
           cur->next[i]) { // Step right //here: cur->next[0], or cur->next[i]??
      cur = (sl_node_t *)unset_mark((uintptr_t)cur->next[i]);
      if (!cur->deleted)
        scanlen--;
    }

    // TODO: This is probably a reasonable condition to become a 'cleaner' since
  ...
  while (cur->deleted && cur->next[0]) {
    cur = (sl_node_t *)unset_mark(
        (uintptr_t)cur->next[0]); // Find first non-deleted node
  }
