...
node__t *harris_search_ins_move(intset_t *set, int idx, int zone, node__t* start_node, node__t **left_node, node__t* starting_last_ptr, LeaderLargest* last_ptr, DebugCounterSlot* num_traversed) {
	node__t *left_node_next, *right_node, *cur_left_node, *cur_left_node_next;
	left_node_next = set->head;
 ...
 search_again:
	do {
		...
		num_traversed->count = num_traversed->count + cnt_traversed;
...
node__t *harris_search(intset_t *set, k_t key, node__t **left_node, DebugCounterSlot* num_traversed) { // left_node init to point to head
...
void harris_search_physdel(intset_t *set, node__t* search_node, DebugCounterSlot* num_traversed) { // left_node init to point to head
 ...
 search_again:
	do {
		...
		num_traversed->count = num_traversed->count + count_traversed;
 ...
 */
int harris_insert(intset_t *set, LeaderLargest* last_ptr, int idx, int zone, k_t key, val__t val, DebugCounterSlot* num_traversed) {
	node__t *newnode, *right_node, *left_node;
	left_node = set->head;
	
	do {
		right_node = harris_search(set, key, &left_node, num_traversed);
...
int harris_insert_and_move(intset_t *set, LeaderLargest* last_ptr, int idx, int zone, k_t* key_rem, k_t key_ins, val__t val, DebugCounterSlot* num_traversed) {
	...
	// insert the new node
	do {
		right_node = harris_search(set, key_ins, &left_node, num_traversed);
	...
	do {
		right_node = harris_search_ins_move(set, idx, zone, newnode, &left_node, starting_last_ptr, last_ptr, num_traversed); // todo: if we retry (below) should reset left_node and newnode ?
	...
	*key_rem = right_node->key;
	...
	if (!__sync_bool_compare_and_swap(&left_node->next, right_node, right_node_next)) {
		harris_search_physdel(set, right_node, num_traversed);
	}
	return ret_val;
}
...
val__t harris_delete_idx(intset_t *set, int idx, int zone, k_t* key) {
	...
	*key = right_node->key;
	...
	if (!__sync_bool_compare_and_swap(&left_node->next, right_node, right_node_next)) {
		harris_search_physdel(set, right_node, NULL);
	}
	return ret_val;
}
