...
sl_node_t *sl_new_simple_node_val(slkey_t key, val_t val, int toplevel,
  ...
  if (transactional) {
    node =
        //(sl_node_t *)MALLOC(sizeof(sl_node_t) + toplevel * sizeof(sl_node_t *));
        (sl_node_t *)malloc(sizeof(sl_node_t) + toplevel * sizeof(sl_node_t *));
  } else {
    /* use *levelmax instead of toplevel in order to be able to use the ssalloc
     * allocator*/
...
//   /* 1 <= level <= *levelmax */
//   if (level > *levelmax) {
//     std::cout << "level: " << level << ", levelmax: " << *levelmax << "\n";
//   }
